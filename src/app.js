...
function applyFilters() {
  ...
  state.filtered = state.players
    .filter((player) => {
      ...
      const priceRange = getPriceRange(player, platform);
      const haystack = normalize(`${player.name} ${player.positions.join(" ")} ${player.foot}`);
      return price > 0
...
function sortPlayers(platform) {
  return (a, b) => {
    switch (els.sortBy.value) {
      ...
      case "range-min-asc":
        return getRangeMin(a, platform) - getRangeMin(b, platform);
      case "range-min-desc":
        return getRangeMin(b, platform) - getRangeMin(a, platform);
      case "range-max-asc":
        return getRangeMax(a, platform) - getRangeMax(b, platform);
      case "range-max-desc":
        return getRangeMax(b, platform) - getRangeMax(a, platform);
      case "name-asc":
...
function renderSummary(platform) {
  ...
  const avgRating = average(state.filtered.map((player) => player.rating));
  ...
  els.avgRating.textContent = avgRating ? avgRating.toFixed(1) : "0";
...
function renderTable() {
  ...
  els.playersBody.innerHTML = state.filtered.map((player) => `
    <tr>
      ...
      <td>${player.addedOn || "-"}</td>
      <td><strong class="mainPrice">${formatCoins(getPrimaryPrice(player, els.platform.value))}</strong></td>
      <td>${formatRange(getPriceRange(player, els.platform.value))}</td>
    </tr>
  `).join("");
}

function statLine(player) {
  const labels = player.positionGroup === "GK"
    ? ["div", "han", "kic", "ref", "spd", "pos"]
    : ["pac", "sho", "pas", "dri", "def", "phy"];
  return labels.map((key) => `${key.toUpperCase()} ${player.stats[key] ?? "-"}`).join(" · ");
}

function valueScore(player, platform) {
  const price = getPrimaryPrice(player, platform) || 1;
  return (player.rating * player.rating) / price;
}

function getPrimaryPrice(player, platform) {
  return player.primaryPrice?.[platform] ?? player.prices?.[platform] ?? 0;
}

function getPriceRange(player, platform) {
  return player.priceRange?.[platform] ?? player.priceRange ?? null;
}

function getRangeMin(player, platform) {
  return getPriceRange(player, platform)?.min ?? Number.MAX_SAFE_INTEGER;
}

function getRangeMax(player, platform) {
  return getPriceRange(player, platform)?.max ?? Number.MAX_SAFE_INTEGER;
}
